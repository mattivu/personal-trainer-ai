...
function NutritionIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6">
      ...
      />
      <path
        d="M10.4 17.45c1.6-2.38 3.92-4.56 7.3-6.2"
        stroke="currentColor"
        strokeWidth={NAV_ICON_STROKE}
        strokeLinecap="round"
      />
...
export function AppBottomNav() {
  ...
  return (
    <nav
      ...
      className="app-bottom-nav-frame pointer-events-none fixed bottom-0 left-1/2 z-50 w-full max-w-[var(--app-shell-outer-width)] -translate-x-1/2 pb-[env(safe-area-inset-bottom,0px)]"
    >
      <div className="app-shell-nav pointer-events-auto rounded-t-[26px] border-x border-t border-white/8 bg-[rgba(10,13,13,0.94)] px-2 pt-2 pb-2 shadow-[0_-10px_28px_rgba(0,0,0,0.24)] backdrop-blur-[18px]">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex min-h-[58px] flex-col items-center justify-center rounded-[18px] px-1 py-1.5 text-center text-[10px] font-semibold transition-colors duration-200 ${
                  active
              ...
              >
                <span
                  className={`relative flex h-8 w-8 items-center justify-center overflow-visible rounded-[14px] transition-colors duration-200 ${
                    featured
                      ? "bg-transparent"
                      : active
                ...
                <span className="mt-1.5 tracking-[0.01em]">{item.label}</span>
              </Link>
            );
          })}
        </div>
